...
4. A "performance" report is created with; cpu usage, virtual memory usage, data sent, bandwidth, and 
   number of ports open, with username and timestamp.

5. The data points are exported as a csv with labels, easily imported/reviewed in Excel, called performanceTimeStamp.csv (filename = performance + timestamp), over time a frame of reference for suspected intrusions.
...
7. Both reports are saved to the USER's "downloads" directory (both Linx and Windows) as CSV's.
...
def exit_collecting():
    global flag
    try:
        val = int(input("Data collection commenced", "Type 0 to exit data collection mode:  "))
        if val == 0:
            flag = 0
